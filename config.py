...
class Config(BaseModel):
    ...
    POLICY_EPSILON_MAX: float = Field(default=1.0)  # HER: None
    ...
    MAX_EPISODE_STEPS: int = Field(default=10)  # HER: STRING_LENGTH
    REPLAY_BUFFER_SIZE: int = Field(default=1024)  # HER: 1_000_000

    # HER
    HER_ENABLED: bool = Field(default=True)
    HER_MAX_DISTANCE: int = Field(default=3)

    # optimization
    LEARNING_RATE: float = Field(default=0.03)  # HER: 0.001
    ...
    GAMMA: float = Field(default=0.95)  # coefficient on future q values HER: 0.98
